...
# 交叉验证
class MriDataset(Data.Dataset):
    def __init__(self, k_choose=[0], transform=None, normalize=True, is_spacing=False):
        """
        交叉验证
        return dataset\\
        Args: 
    ...
    def __getitem__(self, index):
        ...
        if self.is_spacing is True:
            ...
            if shape[0] != shape_spc[0]:
                # spacing resize
                img = img.resize(
                    size=(shape_spc[0], shape_spc[1]), resample=Image.NEAREST)
            if shape_spc[0] != self.max_size_spc:
                img = transforms.Compose([transforms.CenterCrop(size=self.max_size_spc)])(
                    img)  # centercrop to the max_img size

            center_w = (
                self.global_hw_min_max_spc_world[3] + self.global_hw_min_max_spc_world[2]) / 2
            llength = self.global_hw_min_max_spc_world[1] - \
                self.global_hw_min_max_spc_world[0] + 1
            left = int(round(center_w - llength / 2))
            img = img.crop((
                left,    # left
                self.global_hw_min_max_spc_world[0],    # upper
